...
- [ ] **Phase 5** — 商品情報の充実
  - [x] 5a アレルゲン表示（メタオブジェクト `allergen` + 商品メタフィールド `custom.allergens` / `custom.allergen_contamination` → PDP「アレルゲン情報」行に構造化テーブル + 商品カード小バッジ「アレルゲン情報あり」。コレクション/検索の絞り込みは admin の「検索と発見」アプリ側設定で任意）
