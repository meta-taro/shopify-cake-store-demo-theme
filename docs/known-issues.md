...
## 11. アレルゲン表示まわりの割り切り（Phase 5a）

### 内容

- `collapsible_tab` ブロックに追加した `show_allergens` チェックのラベルは、セクション schema に **直接日本語文字列**で書いている（`t:` 翻訳キーを使っていない）。
- 商品カードのバッジは「アレルゲン情報あり」の有無のみ（「○品目不使用」のような不使用表示はやらない）。
- コレクションの「アレルゲン」絞り込みは任意。メタオブジェクト参照型が「検索と発見」アプリのフィルター候補に出ない構成もある。

### 理由

- 翻訳キーにすると `MatchingTranslations` が 20 言語すべての `*.schema.json` への同期を要求し、2 → 多数にエラーが増える（§1 と同じ理由）。学習デモなので literal string で許容。
- 「不使用」表示は、配合変更・コンタミの可能性まで含めた厳密な保証が必要で、デモのダミーデータでは安全に作れない（誤情報リスク）。「含む」側だけ出す。
- 絞り込みは「検索と発見」アプリ側の仕様に依存し、テーマ単体では完結しない。`snippets/facets.liquid` は metafield フィルターを汎用描画するのでテーマ変更は不要。

### 対応するなら

- ラベルを `t:` 化するなら全 20 言語の `*.schema.json` に `sections.main-product.blocks.collapsible_tab.settings.show_allergens.label` / `.info` を追加
- 絞り込みを確実にしたい場合は `custom.allergens` を 1 行テキストのリスト型に変更（フィルター適性が高い）、または別途テキストリストのメタフィールドを併設
