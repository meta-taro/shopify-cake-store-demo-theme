...
| 店舗情報 | `アトリエのご案内` | `store-info` | `store-info` |
...
- 店舗情報ページの `image-with-text` セクションは画像を空のまま push しています。アトリエ写真ができたらテーマカスタマイザで画像をアップロードしてください。
...
3. 「メニュー項目を追加 / Add menu item」を 4 回繰り返し、以下を追加:
   ...
   | 店舗情報 | アトリエのご案内(`/pages/store-info`) |

4. 並び順をドラッグで調整(推奨: お届けについて → よくあるご質問 → 店舗情報 → 特商法)
5. 保存
...
| 店舗情報 | `/pages/store-info` | Sweet Atelier の所在地・営業時間・ご連絡先。アトリエでの受け取り、店頭販売についてもご案内します。 |
