...
| 区分 | `category` | 1行テキスト | ○ | 値は `特定原材料` または `推奨表示` のどちらか（表の「区分」列に出る） |
| アイコン | `icon` | ファイル参照（画像 1点） | ― | 任意。設定すると表の名称の左に 24×24 で出る |

> 「表示名」には `name` フィールドを指定する。

### 9-2. アレルゲンのエントリーを作成

メタオブジェクト定義「アレルゲン」→ **エントリーを追加** で、下記の **デモで実際に使う 6 件だけ**作れば OK（後から増やしてよい）。
...
> 参考: 日本の食品表示法では「特定原材料 8 品目」（卵・乳・小麦・えび・かに・そば・落花生・くるみ）と「推奨表示 20 品目」（アーモンド・あわび・いか・いくら・オレンジ・カシューナッツ・キウイフルーツ・牛肉・ごま・さけ・さば・大豆・鶏肉・バナナ・豚肉・まつたけ・もも・やまいも・りんご・ゼラチン）。本番運用時は商品に合わせて追加する。
