...
商品ページ（PDP）の「アレルゲン情報」行の中に、構造化されたアレルゲン表を表示する。
データは **metaobject（アレルゲン定義）+ 商品メタフィールド** で持つ。
「区分」（特定原材料 / 推奨表示）はテーマ側で食品表示法の固定リストから自動判定するため、メタオブジェクトに区分フィールドは不要。

> ⚠️ Shopify がケーキ系カテゴリーに自動で付ける「カテゴリーメタフィールド」の中に **標準の「アレルゲン情報」フィールド**（EU 系の分類）と「フレーバー」がある。本デモはこれらは**使わない**（空欄でよい）。下記で作る `custom.allergens` を使う。

### 9-1. メタオブジェクト定義「アレルゲン」を作成
...
|---|---|
| 名前 | アレルゲン |
| タイプ（API 用識別子） | `allergen` |

フィールド構成:

| フィールド名 | キー | タイプ | 必須 | 備考 |
|---|---|---|---|---|
| 名称 | `name` | 1行テキスト | ○ | 表に出る名前。例:「卵」「乳」。これを「表示名」に設定 |
| アイコン | `icon` | ファイル参照（画像 1点） | ― | 任意。設定すると表の名称の左に 24×24 で出る。埋めなくてよい |

> 「表示名」には `name` フィールドを指定する。

### 9-2. アレルゲンのエントリーを作成

メタオブジェクト定義「アレルゲン」→ **エントリーを追加** で、下記の **デモで実際に使う 6 件だけ**作れば OK（「名称」にこれを入れるだけ。後から増やしてよい）。
...
> テーマ側は「卵・乳・小麦・えび・かに・そば・落花生・くるみ」を **特定原材料**、それ以外を **推奨表示** として表の「区分」列に出す（日本の食品表示法の 8 品目に対応）。「名称」はこの 8 品目の表記とぴったり一致させること（例: 「乳成分」ではなく「乳」）。
> 参考: 推奨表示 20 品目 = アーモンド・あわび・いか・いくら・オレンジ・カシューナッツ・キウイフルーツ・牛肉・ごま・さけ・さば・大豆・鶏肉・バナナ・豚肉・まつたけ・もも・やまいも・りんご・ゼラチン。本番運用時は商品に合わせて追加する。

### 9-3. 商品メタフィールド定義を 2 つ作成

管理画面 → **設定 → カスタムデータ → 商品 → 定義を追加**

| 名前 | 名前空間とキー | タイプ |
|---|---|---|
| アレルゲン | `custom.allergens` | メタオブジェクト「アレルゲン」への参照（**リスト**） |
| コンタミネーション注記 | `custom.allergen_contamination` | 複数行テキスト |

> `custom.allergens` は「リスト」にすること（単一参照ではない）。表に出る順序は、商品側でこのリストに並べた順になる。
