...
データは **Shopify 提供の「アレルゲン情報」メタオブジェクト + 商品メタフィールド** で持つ。
「区分」（特定原材料 / 推奨表示）はテーマ側で食品表示法の固定リストから自動判定する（メタオブジェクト側で持つ必要なし）。

### 9-1. 「アレルゲン情報」メタオブジェクトのエントリーを作成

管理画面 → **設定 → カスタムデータ → メタオブジェクト → 「アレルゲン情報」（Shopify 提供）→ エントリーを追加**

> 新規でメタオブジェクト定義を作る必要はない。Shopify がケーキ系カテゴリー向けに最初から用意している「アレルゲン情報」メタオブジェクトをそのまま使う。

各エントリーで入力するのは 2 つ:
- **ラベル（表示名）**: 商品ページの表に出る文字。ここに表示したい名前を入れる
- **ベースアレルゲン情報**: Shopify 内部の分類タグ（EU 基準。表示には一切出ない）。一番近いものを選ぶ

下記の **6 件だけ**作れば OK（後から増やしてよい）:

| ラベル（これが表示される） | ベースアレルゲン情報（近いものを選ぶ） |
|---|---|
| 卵 | 卵 |
| 乳 | ミルク |
| 小麦 | 小麦 |
...
> テーマ側は **ラベル**を読み、「卵・乳・小麦・えび・かに・そば・落花生・くるみ」を **特定原材料**、それ以外を **推奨表示** として表の「区分」列に出す（日本の食品表示法の 8 品目に対応）。なのでラベルはこの 8 品目の表記とぴったり一致させること（例: 「乳成分」ではなく「乳」）。くるみとアーモンドはベースが同じ「ナッツ」になるが、ラベルが違うので問題ない。
> 参考: 推奨表示 20 品目 = アーモンド・あわび・いか・いくら・オレンジ・カシューナッツ・キウイフルーツ・牛肉・ごま・さけ・さば・大豆・鶏肉・バナナ・豚肉・まつたけ・もも・やまいも・りんご・ゼラチン。本番運用時は商品に合わせて追加する。

### 9-2. 商品メタフィールド定義を 2 つ作成

管理画面 → **設定 → カスタムデータ → 商品 → 定義を追加**

| 名前 | 名前空間とキー | タイプ |
|---|---|---|
| アレルゲン | `custom.allergens` | メタオブジェクト「アレルゲン情報」への参照（**リスト**） |
| コンタミネーション注記 | `custom.allergen_contamination` | 複数行テキスト |

> `custom.allergens` は「リスト」にすること（単一参照ではない）。参照先には 9-1 で使った Shopify 提供の「アレルゲン情報」メタオブジェクトを選ぶ。表に出る順序は、商品側でこのリストに並べた順になる。
> ※ ケーキ系カテゴリーに最初から付いている「カテゴリーメタフィールド」の標準「アレルゲン情報」「フレーバー」は本デモでは使わない（空欄でよい）。商品ページに「アレルゲン情報」欄が 2 つ並ぶ形になる（標準＝カテゴリーメタフィールド欄／こちら＝メタフィールド欄）。フレーバーはアレルギーとは無関係の味系統タグ。

### 9-4. 各商品にアレルゲンを設定

各商品の編集画面 → 下までスクロール →「メタフィールド」セクションで設定。
...
> 上記はデモ用の架空の配合に基づく例。テーマ側は商品説明欄や「商品仕様」行の原材料記載とは独立してこのメタフィールドのみを参照する。
...
### 9-6. （任意）コレクションでアレルゲンを絞り込み

「検索と発見」アプリ → **フィルター** → `custom.allergens` を追加すると、コレクションページの絞り込みに「アレルゲン」が出る（テーマ側の `snippets/facets.liquid` は metafield フィルターを汎用的に描画するため変更不要）。
※ メタオブジェクト参照型がフィルター候補に出ない場合は、`custom.allergens` を 1行テキストのリスト型に変更するか、別途テキストリストのメタフィールドを併設する。本デモでは絞り込みは任意。

### 注意事項

- アレルゲン表のスタイルは `assets/component-allergen-table.css`、描画は `snippets/product-allergen-table.liquid`
- メタフィールド未設定の商品は「アレルゲン情報は現在準備中です」と表示される（エラーにはならない）
- 商品カードのバッジ「アレルゲン情報あり」は `custom.allergens` が設定されている商品にのみ出る（ただし `人気 / 季節限定 / 新商品` バッジが優先）
