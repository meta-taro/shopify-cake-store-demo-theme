...
- [ ] ブラウザタブに favicon が表示される（SA モノグラム／くすみピンク背景。`/favicon.ico` 404 でなく `assets/favicon.svg` が読まれる。admin で別の favicon をアップロードすればそちらが優先）
...
### Phase 9: 回遊・パーソナライズ（2026-05-21 完了・フロントエンドのみ）

> テーマ側のみで完結（Shopify 設定・アプリ・メタフィールド不要）。すべて `localStorage`（名前空間 `sweet-atelier:`）。
> Playwright（chromium）で自動検証済み。実機目視はローカル `http://127.0.0.1:9292` で確認。

#### 9a 最近見た商品
- [x] 商品ページ閲覧で `localStorage` キー `sweet-atelier:recently-viewed` に handle が記録される（最大 12 件・最新が先頭）
- [x] 別商品・トップ・コレクション等で「最近見た商品」セクションにカードが表示される（現在の商品は除外）
- [x] 履歴が空のときはセクション全体が非表示（見出しごと出ない）

#### 9b ウィッシュリスト
- [x] コレクションカード右上のハートをクリックしてもカードリンクに奪われず、その場でトグルする（`card-wrapper` 直下配置で `::after` スタッキング回避）
- [x] トグルで `localStorage` キー `sweet-atelier:wishlist` に商品ごとの正しい handle が保存される（コレクションページで全件 "all" になる不具合を修正済み）
- [x] `aria-pressed` がお気に入り状態を反映（追加で true）
- [x] ヘッダーのハートに件数バッジが出て、追加・解除で即時同期（0 件で非表示）
- [x] PDP のお気に入りボタンがテキスト（追加 ⇄ 削除）と `aria-pressed` を切り替え、ヘッダー件数とも同期
- [x] `wishlist.js` が二重ロードされても `const` 再宣言エラーが出ない（IIFE + `customElements.get` ガード）
- [ ] お気に入り一覧ページ（`/pages/wishlist`）で保存済み商品のカードが並ぶ／一覧でトグル解除するとその場で消える ※ **管理画面でページ作成が前提**（テンプレート `page.wishlist`・ハンドル `wishlist`、`docs/admin-setup.md §12`）。未作成のため最終目視は未実施

#### 9c 送料シミュレーター
- [x] 配送ページ（`/pages/delivery`）の「配送について」直後に送料シミュレーターが表示される
- [x] 都道府県セレクトに 47 都道府県が地域 optgroup で並ぶ（プレースホルダー含め 48 option）
- [x] 都道府県を選ぶと配送区分と送料が出る（本州 ¥980 / 北海道・四国・九州 ¥1,280 / 沖縄 ¥1,800）
- [x] 注文金額 ¥10,000 以上で送料無料表示（沖縄は対象外として明示）／未達なら「あと ¥N で送料無料」
- [x] 選択した都道府県が `localStorage` キー `sweet-atelier:shipping-pref` に保存され、リロードで復元される
- [x] 「※ 表示は目安です。確定送料は購入手続き画面でご確認ください。」の注記を表示
- [x] `shopify theme check` が baseline（2 errors / 9 warnings）を超えていない

> **admin 前提（user 作業・未実施）**: お気に入り一覧の表示には管理画面でページ作成が必要（§12）。送料シミュレーターと最近見た商品・カード／PDP のお気に入りトグルは admin 作業なしで動作。

### Phase 16 — スプレッドシートでの商品運用

#### 16a〜c CSV 往復
- [ ] 管理画面 → 商品 → エクスポートで全 5 商品を CSV ダウンロード（UTF-8 / カンマ）
- [ ] Google Sheets に「インポート → カンマ区切り」で取り込み → 文字化けなし
- [ ] 価格・タグなどを編集 → UTF-8 で CSV ダウンロード → 管理画面でインポート → 上書き反映を目視
- [ ] サンプル CSV (`docs/samples/products-sample.csv`) を別ストアにインポートしても 5 商品・12 バリアントが作成できる
- [ ] `Status=Draft` の行は店頭に出ない／`Status=Active` かつ `Published on online store=TRUE` で表示される

#### 16d Apps Script からの直接同期（`docs/samples/google-sheets/`）
- [ ] 新規 GAS プロジェクトに `Code.gs` と `download.html` を貼り付け → シートをリロードで［Shopify］メニューが表示される
- [ ] ［接続設定］でドメインとトークンを保存（シート本体には残らない／Script Properties にだけ保管）
- [ ] ［接続テスト］でストア名が返る
- [ ] 初回 ［ストアに同期］で 5 商品・12 バリアントが新規作成され、各行に「✓ 新規 …」が記録される
- [ ] 価格／在庫を 1 セル書き換えて再同期 → 該当行が「✓ 更新 …」になり、新規作成にならない（handle 一致のため）
- [ ] 再同期で**画像が増殖しない**（既存 media 数が 1 のまま。新規作成時のみ `productCreateMedia` を呼ぶ実装）
- [ ] `公開状態=下書き` の行は管理画面で Draft、`Published on online store` が FALSE
- [ ] わざと不正値（`価格 = "高め"` 等）を入れると「同期結果」列に `✗ <理由>` が出る
- [ ] トークンを無効化／削除して再同期すると「認証エラー (401)」が「同期結果」に出る
- [ ] ［取込用 CSV をダウンロード（フォールバック）］が UTF-8+BOM の Shopify 標準 CSV を生成し、管理画面でそのまま import できる
